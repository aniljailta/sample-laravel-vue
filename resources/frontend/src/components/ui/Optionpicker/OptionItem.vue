    ...
    <tr>
            ...
            <td>
                ...
                <colorpicker v-if="buildingOption.option.allowableColors && buildingOption.option.allowableColors.length > 0"
                             ...
                             v-bind:color="buildingOption.color || buildingOption.defaultColor">
                </colorpicker>
            </td>
            <td class="nowrap">
            ...
            <td class="text-left">
                <div class="btn-group">
                    <div class="form-group">
                        <div class="input-group input-group-xs" style="width: 100px">
                            <div class="input-group-btn btn-group-xs" role="group">
                                <button type="button" class="btn btn-default" v-on:click="increaseOption(buildingOption)" v-bind:disabled="!canIncrease">
                            ...
                            <input number
                                   ...
                                   initial="off"
                                   style="width: 50px"
                                   v-bind:max="buildingOption.option.constraintType == 'less_than' && buildingOption.minQuantity || null"
                    ...
                    <span v-if="buildingOption.minQuantity && buildingOption.option.constraintType === 'equal_to'" class="label label-default nowrap">A Quantity of {{ buildingOption.minQuantity }} is Required.</span>
                    <span v-if="buildingOption.minQuantity && buildingOption.option.constraintType === 'less_than'" class="label label-default nowrap">Max Quantity is {{ buildingOption.minQuantity }}.</span>
            </td>
...
<script type="text/babel">
    ...
    export default {
        ...
        created() {
            this.$watch('color', this.onChangeColor, {deep: true})
            this.$nextTick(() => {
                if (this.color) {
        ...
        computed: {
            ...
            canDecrease() {
                if (this.bo.option.constraintType) {
                    if (_.isArray(this.bo.parentOptions) && this.bo.parentOptions.length >= this.bo.quantity) {
                        return false
                    }
                    if (this.bo.option.constraintType === 'equal_to' && this.bo.quantity === this.bo.minQuantity) {
                        return false
                    }
                    if (this.bo.option.constraintType === 'less_than' && this.bo.quantity <= this.bo.minQuantity) {
                        return true
                    }
                    return false
                }
