<template>
    ...
    <div class="form-horizontal">
        ...
        <div class="row" v-show="rtoSignaturePending || successMessageVisible">
            <div class="col-xs-offset-1 col-xs-10 alert text-center"
                 :class="{ 'alert-warning': rtoSignaturePending, 'alert-success': successMessageVisible }">
                {{ message }}
             </div>
        </div>
            ...
            <div class="form-group">
                <div class="row col-xs-12" v-if="curItem.id">
                    ...
                    <div class="col-xs-12 col-md-3">
                        ...
                        <div id="order_status">
                            <order-status-label :status="curItem.order.status"></order-status-label>
                        </div>
                    </div>
                    <div class="col-xs-12 col-md-3">
                        <button class="btn btn-default"
                                @click.prevent="getRtoSignatureStatus"
                                v-show="rtoSignaturePending">
                            <i class="fa fa-circle-o-notch fa-spin text-muted" v-show="loading"></i> Check Signature Status
                        </button>
                    </div>
                </div>
                ...
                <div class="row col-xs-12 col-sm-4">
                    <div class="col-xs-12" v-if="curItem.id && curItem.building !== null">
                        ...
                        <div id="building">
                            <dl class="">
                                    <dt>Status</dt><dd><building-status-label :status="curItem.building.lastStatus.buildingStatus" v-if="curItem.building.lastStatus"></building-status-label></dd>
...
<script type="text/babel">
    ...
    import apiOrder from 'src/api/orders'
    ...
    export default {
        ...
        data() {
            return {
                ...
                message: 'This sale is currently pending a signature by the RTO company. The sale must be signed by all parties to be invoiced.',
                loading: false,
        ...
        methods: {
            ...
            getRtoSignatureStatus() {
                this.loading = true
                this.$emit('data-process-reset')
                apiOrder.getRtoSignatureStatus({id: this.curItem.order.uuid})
                    .then(response => {
                        if (response.body.rtoSignatureReceived) {
                            this.message = 'RTO company signed the document, now the sale can be invoiced.'
                            this.rtoSignaturePending = false
            ...
            submit() {
                ...
                return this.save({ item: form })
                    ...
                    .catch(response => {
                        if (response.body[1] && response.body[1] === 'signature_refresh_required') {
                            this.message = response.body[0]
                        }
                ...
                if (this.id) {
                    apiSales.get({
                        ...
                        .then(response => {
                            ...
                            this.rtoSignaturePending = item.rtoSignaturePending
